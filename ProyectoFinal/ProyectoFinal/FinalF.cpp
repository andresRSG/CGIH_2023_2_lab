...
bool playAmbientSound = true;
bool playTiburon = false;
bool playSharkSound = false;
...
// camera
Camera camera(glm::vec3(940.0f, 7.0f, -150.0f));
...
void wolfHowl() {
	bool played = PlaySound(L"aullido.wav", NULL, SND_ASYNC);
}

void sharkSound() {
	bool played = PlaySound(L"tiburon.wav", NULL, SND_ASYNC);
}
////////////////////////////////////////

/////////CÁMARA DE SEGURIDAD MOV////////////////////

// Definición de la enumeración
enum CameraState { NORMAL, CAMERA1, CAMERA2, CAMERA3 };
...
int main()
{
	...
	Model camara1("resources/objects/Camara/camara.obj");
	Model camara2("resources/objects/Camara/camara.obj");
	Model camara3("resources/objects/Camara/camara.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (playL)
		{
			// Realizar la animación del lobo

			// Verificar si se debe reproducir el sonido del aullido del lobo
			if (playSound)
			{
				wolfHowl();  // Reproducir el aullido del lobo
				playSound = false;
				music();  // Reiniciar el audio de ambiente
			}

			// Resto del código de la animación del lobo
		}

		// Verificar si se debe reproducir el audio de ambiente
		if (playAmbientSound)
		{
			music();  // Reproducir el audio de ambiente
			playAmbientSound = false;
		}

		if (playTiburon) {
			// Realizar la animación del tiburón

			// Verificar si se debe reproducir el sonido del tiburón
			if (playSharkSound) {
				sharkSound();  // Reproducir el audio del tiburón
				playSharkSound = false;
			}

			// Resto del código de la animación del tiburón
		}

		// Reproducir el audio de ambiente
		music();
		...
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(-77.0f, 140.0f, -605.0f));
		model = glm::scale(model, glm::vec3(0.3f));
		model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		staticShader.setMat4("model", model);
		camara1.Draw(staticShader);

		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(538.0f, 140.0f, 165.0f));
		model = glm::scale(model, glm::vec3(0.3f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		staticShader.setMat4("model", model);
		camara2.Draw(staticShader);

		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(538.0f, 140.0f, -480.0f));
		model = glm::scale(model, glm::vec3(0.3f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		staticShader.setMat4("model", model);
		camara3.Draw(staticShader);
...
void my_input(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	// Verificar si se oprime la tecla "C"
	if (key == GLFW_KEY_C && action == GLFW_PRESS) {
		cameraCount++;  // Incrementar el contador de cámaras

		// Actualizar el estado de la cámara según el contador
		if (cameraCount == 1) {
			cameraState = CAMERA1;
		}
		...
		// Actualizar la posición de la cámara según el estado actual
		if (cameraState == NORMAL) {
			camera.Position = glm::vec3(940.0f, 15.0f, -360.0f);
		}
		else if (cameraState == CAMERA1) {
			camera.Position = glm::vec3(250.0f, 160.0f, -360.0f);
		}
		else if (cameraState == CAMERA2) {
			camera.Position = glm::vec3(200.0f, 160.0f, -100.0f);
		}
		else if (cameraState == CAMERA3) {
			camera.Position = glm::vec3(200.0f, 160.0f, -1000.0f);
		}
	}
	...
	if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) {
		...
		// Verificar si se debe reproducir el sonido del tiburón
		if (sharkAnimate && !playSharkSound) {
			playTiburon = true;
			playSharkSound = true;
			sharkSound();  // Reproducir el audio del tiburón
		}
